...
// #include "def.h"						// For BYTE define
/*
int	Hex_Dump(unsigned char* buff, int nLen);
bool FeeCalculation(char* szFillingAmt, char* szFee, char* szTran_kubun);
*/
// S-BOX
...
#define SeedRound(L0, L1, R0, R1, K) {             \
    ...
    T1 = R1 ^ (K)[1];                              \
    T1 ^= T0;                                      \
    T1 = SS0[GetB0(T1)&0x0ff] ^ SS1[GetB1(T1)&0x0ff] ^         \
         SS2[GetB2(T1)&0x0ff] ^ SS3[GetB3(T1)&0x0ff];          \
    T0 += T1;                                      \
    T0 = SS0[GetB0(T0)&0x0ff] ^ SS1[GetB1(T0)&0x0ff] ^         \
         SS2[GetB2(T0)&0x0ff] ^ SS3[GetB3(T0)&0x0ff];          \
    T1 += T0;                                      \
    T1 = SS0[GetB0(T1)&0x0ff] ^ SS1[GetB1(T1)&0x0ff] ^         \
         SS2[GetB2(T1)&0x0ff] ^ SS3[GetB3(T1)&0x0ff];          \
    T0 += T1;                                      \
...
/* Test JHLEee 2022-12-16
#define EndianChange(dwS)								\
	( (ROTL((dwS),  8) & (DWORD)0x00ff00ff) |			\
	(ROTL((dwS), 24) & (DWORD)0xff00ff00) )				\
*/

// ((((dwS) << (8)) | (((dwS) >> (32-(8)))&0x000000ff)) & 0x00ff00ff) | ((((dwS) << (24)) | (((dwS) >> (32-(24)))&0x00ffffff)) & 0xff00ff00)
#define EndianChange(dwS)								\
...
int SEED_CBC_Decrypt( IN BYTE *pbszUserKey, IN BYTE *pbszIV, IN BYTE *pbszCipherText, IN int nCipherTextLen, OUT BYTE *result )
{
	...
	{
		printf("\n\n  nCipherTextLen: %d, BLOCK_SIZE_SEED: %d, RE: %d", nCipherTextLen, BLOCK_SIZE_SEED, (nCipherTextLen % BLOCK_SIZE_SEED));
		return 0;
	}
	printf("\n\n\n  OK__nCipherTextLen: %d, BLOCK_SIZE_SEED: %d, RE: %d", nCipherTextLen, BLOCK_SIZE_SEED, (nCipherTextLen % BLOCK_SIZE_SEED));
...
int main()
{
	...
	// private final String param1 = "0F 43 57 DA 59 A4 BC 5E 64 DB AA B9 8D CB 56 BA";
	// private final String param2 = "00 00 00 00 000000000000000000000000";
	...
	//BYTE plainText[] = "I1J0I0J0I1H9F3E400000100000000000000 000000000001000000000010010000001384F641365346B0DB0101041000129336000000000000000000000000000000002710070101048E4A16C6I1J0I0J0I1H9F3E400000100000000000000 000000000001000000000010010000001384F641365346B0DB0101041000129336000000000000000000000000000000002710070101048E4A16C6I1J0I0J0I1H9F3E400000100000000000000 000000000001000000000010010000001384F641365346B0DB0101041000129336000000000000000000000000000000002710070101048E4A16C6";
	BYTE plainText[] = "1234567898";
	...
	BYTE pbszDecMsg[278] = {};
													
		BYTE szCipher[] =	{ 
		 };
	
	BYTE szstr[] = {};
	...
	printf("\n\nSOURCE Length : %d", sizeof(plainText));
	...
	printf ("\nEncryption....\n");
			...
			message_length = nPlainTextLen;			// 암호화 대상 길이
			SEED_CBC_init( &info, KISA_ENCRYPT, pbszUserKey, pbszIV );	
			printf ("\nEncryption....1\n");
	...
	for (i=0;i<nCipherTextLen;i++)	{printf("%c ",CipherBytes[i]);}

	printf ("\n\nDecryption....");

	// Decryption Algorithm //
	
	//printf("\n\n strlen-szstr: %d", strlen((char*)szstr));
	//nCipherTextLen = 305;		// Insert
	//printf("\n\n sizeof-szstr: %d", sizeof(szstr));
	//memcpy(pbszCipherText, szstr, nCipherTextLen);
	...
	{
		printf("\n\n OK nCipherTextLen: %d, BLOCK_SIZE_SEED: %d, RE: %d", nCipherTextLen, BLOCK_SIZE_SEED, (nCipherTextLen % BLOCK_SIZE_SEED));
		SEED_CBC_init( &info, KISA_DECRYPT, pbszUserKey, pbszIV );
...
bool FeeCalculation(char* szFillingAmt, char* szFee, char* szTran_kubun)
{
	...
	nFillingAmt = strtol(szFillingAmt, NULL, 16);		// 충전 요청금액을 10진수로 변환
	
	
	// 결제 수단 별 수수료 율 계산
	if(!strncmp(szTran_kubun, "01", 2))			// 현금
		nfee = nFillingAmt * 0.005;				// 충전 총액의 0.5%
	else if(!strncmp(szTran_kubun, "02", 2) || !strncmp(szTran_kubun, "05", 2))	// 02: 신용카드, 05: 현금 IC
		nfee = nFillingAmt * 0.003;				// 충전 총액의 0.3%
	else 
	{
		printf("\n\n     TRAN_KUBUN Fail!!!  FillingAmt: %s, tran_kubun: %s, nFillingamt: %d, nfee: %d, szFee: %s",
				szFillingAmt, szTran_kubun, nFillingAmt, nfee, szFee);
		return false;
	}
	
	sprintf(szFee, "%010d", nfee);
	printf("\n\n     Fee Compute, FillingAmt: %s, tran_kubun: %s, nFillingamt: %d, nfee: %d, szFee: %s",
				szFillingAmt, szTran_kubun, nFillingAmt, nfee, szFee);
